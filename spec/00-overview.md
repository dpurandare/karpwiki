...
| 2 | **The wiki is structured, cited, cross-referenced markdown** | Concept/entity/source/comparison pages, `overview.md` hub, `index.md` catalog, append-only `log.md` — same backbone as Karpathy's reference pattern, using standard markdown cross-reference and footnote-citation conventions. |
...
| 6 | **Ingestion is asynchronous and stateful**, modeled as a multi-stage async pipeline (`submitted → classifying → classified → duplicate_check → ingesting → ingested`), extended with `pending_review`, `rejected`, and `error` states (full state machine in [03](03-ingestion-and-review-workflows.md) §1). Nothing the curator does to the wiki happens synchronously with a user's upload. |
...
| 10 | **Dual interface: API and MCP** | The same gateway capabilities are exposed as a conventional REST/GraphQL API (for apps/services) and as an MCP server (for AI agents/IDEs), exposing a resolve-then-retrieve tool pattern (e.g., `wiki_search`/`wiki_list_pages` to resolve, then `wiki_get_page` to retrieve — see [06](06-api-mcp-and-scaling.md) §2). |
...
| [user-guide-contributors.md](user-guide-contributors.md) | *(Optional appendix)* How to submit documents and search as a regular user — submission status, what happens after you submit, page types, feedback |
