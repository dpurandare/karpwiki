...
Credential handling: `connector.credential_ref` is passed to the adapter as-is — no real
secrets-manager resolution exists until step 53 (09 §13), so an adapter today would receive
whatever opaque pointer string an admin configured, not a resolved secret. This mirrors
step 51's own scope boundary for `credential_ref`.
"""
...
class ConnectorAdapter(Protocol):
    ...
    `Authenticator` pattern, per 09 §13's own note that credential resolution (step 53)
    follows that same shape."""

    async def poll(
        self, connector: Connector, credential_ref: str | None
        ...
        the row it's given), and return `(new_or_changed_items, new_cursor)`. Raise
        `ConnectorAuthError` for an authentication failure; any other exception is treated
        as a transient fetch error that simply retries on the connector's next scheduled
        run."""
        ...
...
async def poll_connector(session: AsyncSession, *, connector: Connector) -> list[uuid.UUID]:
    ...
    try:
        items, new_cursor = await adapter.poll(connector, connector.credential_ref)
