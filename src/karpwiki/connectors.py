...
async def list_for_workspace(session: AsyncSession, *, workspace_id: str) -> list[Connector]:
...
async def list_for_workspaces(session: AsyncSession, *, workspace_ids: list[str]) -> list[Connector]:
